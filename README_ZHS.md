...
本mod仍处于早期测试阶段，因此可能存在大量未探明的bug，如果你发现了这些bug，请务必提交issues，我会在闲暇之际进行统一修复。

所有功能均严格限定于 Zombies 游戏模式，在其他任何游戏模式下全部功能均被禁用、不产生任何效果。

模组界面及所有提示信息均提供中英文两种语言，显示语言由客户端设置决定。

![Battle Of AA](https://cdn.modrinth.com/data/cached_images/1ecd2db537000cd24985928098900038a9c1fb35.png)
...
- 每类 ESP 可独立开关，并分别设置两种渲染模式：
  - **常规模式：** 遵守深度测试，实体被方块遮挡时不显示。
  - **穿透模式：** 实体在被方块遮挡时仍保持可见，便于在 PvE 环境下快速定位怪物位置及队友状态。
- 提供全局穿透渲染距离滑条，范围 5 至 200 格，控制穿透模式的有效范围。
- 穿透模式仅适用于 PvE 环境下的怪物与队友定位，不涉及 PvP 对抗场景。
